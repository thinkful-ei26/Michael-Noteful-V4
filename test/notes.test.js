      ...
      return Promise.all([
        ...
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            ...
            expect(item).to.include.all.keys('id', 'title', 'createdAt', 'updatedAt', 'tags','userId','content');
    ...
    it('should return correct search results for a title search', function () {
      ...
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            expect(item).to.be.a('object');
            expect(item).to.include.all.keys('id', 'title', 'createdAt', 'updatedAt', 'tags','userId','content'); // Note: folderId and content are optional
            expect(item.id).to.equal(data[i].id);
            expect(item.title).to.equal(data[i].title);
            expect(item.content).to.equal(data[i].content);
            expect(new Date(item.createdAt)).to.eql(data[i].createdAt);
    ...
    it('should return correct notes', function () {
      ...
      return Note.findOne()
        ...
        .then((res) => {
          ...
          // Note: folderId, tags and content are optional
          expect(res.body).to.include.all.keys('id', 'title', 'createdAt', 'updatedAt', 'tags','userId','content');
    ...
    it('should create and return a new item when provided valid title and content', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.have.all.keys('id', 'title', 'createdAt', 'updatedAt', 'tags','userId','content');
    ...
    it('should return an error when a tag `id` is not valid ', function () {
      ...
      return chai.request(app)
        ...
        .then(res => {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body.message).to.equal('The tags were not valid or owned by the user');
        });
    });
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should update the note when provided a valid title', function () {
      ...
      return Note.findOne()
        ...
        .then(function (res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('id', 'title', 'createdAt', 'updatedAt', 'tags','userId','content');
          expect(res.body.id).to.equal(data.id);
    ...
    it('should update the note when provided a valid folderId', function () {
      ...
      return Promise.all([
        ...
        .then(([folder, note]) => {
          ...
          data = note;
          return chai.request(app)
            .put(`/api/notes/${note.id}`).send(updateItem)
            .set('Authorization', `Bearer ${token}`);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
    ...
    it('should unset a note folderId when provided a empty string', function () {
      ...
      return Note.findOne({ folderId: { $exists: true } })
        .then((note) => {
          data = note;
          return chai.request(app).put(`/api/notes/${note.id}`).send(updateItem).set('Authorization', `Bearer ${token}`);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
          expect(res).to.be.json;
